...
def getUserMsgs(req, uid=None):
    try:
        ...
        if len(chats) == 0 or uid is None:
            return render(req, "msg-detail.html", {"avatar_url": userLogued, "channels": chats})
        ...
        if req.method == 'POST':
            ...
            info.success(req, "Message send!")
            return render(req, "msg-detail.html", {"avatar_url": userLogued, "form": messageForm, "chats": messages, "to": toUser.id, "to_user": f"{toUser.first_name} {toUser.last_name}:", "channels": chats})
